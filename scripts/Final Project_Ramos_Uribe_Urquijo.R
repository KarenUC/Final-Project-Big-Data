...
##Dado que existen accidentes con heridos y con muertos pero sin v√≠ctimas, se asume que si tienen heridos o victimas, al menos tienen una v√≠ctima
...
##Dado que existen otro tipo de "infracciones", se crea una categor√≠a adicional
...
# Partimos nuestra base aleatoriamente. As√≠ garantizamos que se conserve la distribuci√≥n en las marcas.
set.seed(123)
...
#Verificamos que la partici√≥n sea adecuada
...
auto_eps <- function(xdf, min_pts) { # Se introduce un dataset de dos columnas normalizadas.
  ...
  xr <- rescale(x)
  yr <- rescale(y)
  curvatura <- array() # AquÌ se guardar·n las pendientes.
...
#k: el n˙mero mÌnimo de puntos que elegimos
